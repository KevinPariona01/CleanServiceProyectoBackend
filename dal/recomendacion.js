...
//NOMBRE DE LA TABLA
const nombreTabla = 'gen_recomendacion';

const listarRecomendacion = (request, response)=>{
    var obj = valida.validaToken(request)
    if (obj.estado){
        let cadena = `SELECT n_idgen_recomendacion, c_nombre, c_descripcion, n_borrado FROM ${nombreTabla}
                      WHERE n_borrado = 0
        `;
        pool.query(cadena, 
        (error, results)=>{
...
const agregarRecomendacion = (request, response)=>{
...
const actualizarRecomendacion = (request, response)=>{
    let n_idgen_recomendacion = request.body.n_idgen_recomendacion;
    let c_nombre = request.body.c_nombre;
    let c_descripcion = request.body.c_descripcion;
    var obj = valida.validaToken(request)
    if (obj.estado){
        let cadena = `UPDATE ${nombreTabla} 
                      ...
                      WHERE n_idgen_recomendacion = ${n_idgen_recomendacion}
        `;
        pool.query(cadena, 
        (error, results)=>{
            if (error) {
...
const eliminarRecomendacion = (request, response)=>{
    let n_idgen_recomendacion = request.body.n_idgen_recomendacion;
    var obj = valida.validaToken(request)
    if (obj.estado){
        let cadena = `UPDATE ${nombreTabla} 
